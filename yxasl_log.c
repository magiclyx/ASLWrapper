...
#define HSLHANDLE2SLCONTEXT(h) ((_asl_context_ref_)h)

typedef struct __asl_context_{
    aslclient _asl;
    int _log_file_fd;
    int _current_level;
    ...
    const char* _log_dir;
}_asl_context_, *_asl_context_ref_;

static const char* _user_log_dir = "~/Library/Logs";
static const char* _sys_log_dir = "/var/log";


static _asl_context_ref_ _init_asl_log(const char *ident, const char *facility, const char* log_file);
static void _asl_log(_asl_context_ref_ context, int level, const char* msg, void *ctx);
static void _uninit_asl_log(_asl_context_ref_ context);

static void _setDirName(_asl_context_ref_ handle, const char* dirName);
static void _setFileName(_asl_context_ref_ handle, const char* fileName);
...
HSL_Handle YXASL_InitUserLog(const char *ident, const char *facility, const char* dirName, const char* fileName)
{
    ...
    _asl_context_ref_ context = _init_asl_log(ident, facility, buff);
    if(NULL != context){
...
int YXASL_bindLogFile(HSL_Handle handle, const char* dirName, const char* fileName){
    ...
    _asl_context_ref_ context = HSLHANDLE2SLCONTEXT(handle);
...
{
    _asl_context_ref_ context = *handleRef;
...
{
    _asl_context_ref_ context = handle;
    ...
    {       
        ...
        switch (level) {
            ...
            default:
            {
                ...
                YXASL_Log(handle, ASL_LEVEL_WARNING, buff);
            }
...
static _asl_context_ref_ _init_asl_log(const char *ident, const char *facility, const char* log_file)
{
    ...
    _asl_context_ref_ contextRef = NULL;
    
    do
    {
        contextRef = (_asl_context_ref_)malloc(sizeof(_asl_context_));
        if(NULL == contextRef)
            break;
...
static void _setDirName(_asl_context_ref_ context, const char* dirName){    
    strncpy(context->_dirName, dirName, 1024);
    context->_dirName[1024] = '\0';
}
static void _setFileName(_asl_context_ref_ context, const char* fileName){
